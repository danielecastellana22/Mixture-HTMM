classdef SP_BHTMM < handle
    ...
    methods
        ...
        function obj = SP_BHTMM(C_,L_,M_,alpha_A_,alpha_pr_,alpha_b_,alpha_SP_)
        ...
        function init_potentials_from_priori(obj)
            ...
            alpha_A_ = obj.alpha_A;
            alpha_b_ = obj.alpha_b;
            ...
            tab = zeros(C_,L_);
            for l=1:L_
                tab(:,l) = drchrnd(alpha_pr_*ones(1,C_),1) + eps;
            end
            obj.prLeaf.variables = [h_state_pa_ ch_pos_];
            obj.prLeaf.table = log(tab);
            
            
            tab = drchrnd(alpha_SP_*ones(1,L_),1) + eps;
            ...
            for l=1:L_
                for c=1:C_
                    tab(:,c,l) = drchrnd(alpha_A_*ones(1,C_),1) + eps;
                end
            end
            tab(:,bottomCh_,:) = zeros(C_,1,L_);
            ...
            tab = zeros(C_,M_);
            for c=1:C_
                tab(c,:) = drchrnd(alpha_b_*ones(1,M_),1);
            end
            tab(bottomCh_,:) = zeros(1,M_);           
